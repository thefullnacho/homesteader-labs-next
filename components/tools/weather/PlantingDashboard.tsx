...
import { Sprout, Thermometer, Droplets, Clock } from 'lucide-react';
import Typography from "@/components/ui/Typography";
import BrutalistBlock from "@/components/ui/BrutalistBlock";
import Badge from "@/components/ui/Badge";
import type { PlantingIndex } from "@/app/lib/weatherTypes";
import { formatFrostRisk, formatConfidence } from "@/app/lib/plantingIndex";
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {/* FROST ANALYSIS */}
        <BrutalistBlock className="p-6" title="FROST_PROBABILITY_MATRIX" refTag="SNS_ICE_01">
          <div className="space-y-6 mt-4">
            {[
              { label: "7-DAY_CYCLE", risk: index.frostRisk.next7Days },
              { label: "14-DAY_CYCLE", risk: index.frostRisk.next14Days },
              { label: "30-DAY_CYCLE", risk: index.frostRisk.next30Days },
            ].map((period) => (
              <div key={period.label}>
                <div className="flex justify-between items-end mb-2">
                  <Typography variant="small" className="font-mono font-bold uppercase text-[9px] mb-0 opacity-40">{period.label}</Typography>
            ...
            <div className="pt-4 border-t border-border-primary/10 flex justify-between items-center">
              ...
              <Typography variant="small" className="font-mono text-[8px] mb-0 opacity-30 uppercase tracking-widest text-right">
                VAR_THRESHOLD: ±{index.frostRisk.variance}°F
              </Typography>
            </div>
          </div>
        </BrutalistBlock>
        ...
        <div className="space-y-6">
          <BrutalistBlock className="p-5" refTag="SOIL_VITAL_SIGNS">
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-3 opacity-40">
                ...
                <Typography variant="small" className="uppercase font-bold text-[10px] mb-0 tracking-widest font-mono">Soil_Workability</Typography>
              </div>
          ...
          <BrutalistBlock className="p-5" refTag="GROWTH_ENVELOPE">
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-3 opacity-40">
                ...
                <Typography variant="small" className="uppercase font-bold text-[10px] mb-0 tracking-widest font-mono">Sustainable_Window</Typography>
              </div>
              {index.plantingWindow.opens && (
                <span className="text-[10px] font-bold font-mono text-accent uppercase tracking-tighter">
                  {index.plantingWindow.confidence}%_PROB
                </span>
              )}
            </div>
            {index.plantingWindow.opens ? (
              <div className="space-y-1">
                <div className="flex justify-between text-[11px] font-mono uppercase">
                  <span className="opacity-40">Stream_Start:</span>
                  <span className="font-bold">{new Date(index.plantingWindow.opens).toLocaleDateString()}</span>
                </div>
                <div className="flex justify-between text-[11px] font-mono uppercase">
                  <span className="opacity-40">Window_Est:</span>
                  <span className="font-bold text-accent">{index.plantingWindow.days}_CONSEC_DAYS</span>
                </div>
              </div>
            ) : (
              <Typography variant="small" className="opacity-30 italic mb-0 uppercase text-[10px] font-mono">No_Sustainable_Growth_Detected</Typography>
            )}
          </BrutalistBlock>
          ...
          <BrutalistBlock className="p-5 border-green-900/30" refTag="THERMAL_ACCUM">
            <div className="flex justify-between items-center mb-4 opacity-40">
              <Typography variant="small" className="uppercase font-bold text-[10px] mb-0 tracking-widest font-mono">Heat_Unit_Summation</Typography>
            ...
            <div className="w-full h-4 bg-background-secondary border-2 border-border-primary/20 overflow-hidden relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <span className="text-[7px] font-bold font-mono text-white mix-blend-difference uppercase tracking-[0.2em]">Growing_Degree_Days_Sum</span>
              </div>
            </div>
          </BrutalistBlock>
        </div>
      ...
      {/* RECOMMENDATIONS */}
      <BrutalistBlock className="border-accent/40 bg-accent/5 p-6" title="PROTOCOL_DEPLOYMENT_RECS" refTag="OP_ORD_01">
